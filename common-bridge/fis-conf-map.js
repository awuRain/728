...
fis.config.merge({
    ...
    deploy: {
        remote: [{
            ...
            from: '/static',
            subOnly: true,
            to: '/home/lv/webroot/fwmap/upload/dq_promotion/map/static',
            exclude: /.*\.(?:svn|cvs|tar|rar|psd).*/
        }, {
            receiver: deploy + '/static/receiver.php',
            from: '/page',
            subOnly: true,
            to: '/home/lv/webroot/fwmap/upload/dq_promotion/map/',
            exclude: /.*\.(?:svn|cvs|tar|rar|psd).*/
        }]
    }
});
...
fis.config.set('roadmap.path', [{
    ...
    url: '/fwmap/upload/dq_promotion/map/static/pkg/$1'
}, {
    reg: /^\/page\/(.*\.*)$/i,
    isMod: true,
    //发布到/static/js/xxx目录下
    release: '/$1',
    url: '/fwmap/upload/dq_promotion/map/$1'
}, {
    reg: /^\/lib\/(.*\.js)$/i,
    //非组件化
    isMod: false,
    ...
    url: '/fwmap/upload/dq_promotion/map/static/lib/$1'
}, {
    ...
    url: '/fwmap/upload/dq_promotion/map/static/modules/$1/$1' + '.js'
}, {
    //modules目录下的其他文件
    reg: /^\/modules\/(.*)\.(js)$/i,
    //是组件化的，会被jswrapper包装
    ...
    url: '/fwmap/upload/dq_promotion/map/static/modules/$1' + '.js'
}, {
    //widgets
    reg: /^\/widgets\/(.*)\.(js)$/i,
    //是组件化的，会被jswrapper包装
    ...
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1' + '.js',
}, {
    reg: /^\/widgets\/(.*)\.(?:css|less)$/i,
    release: '/static/widgets/$1' + '.css',
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1' + '.css',
}, {
    //图片
    reg: /^\/widgets\/(.*\.(?:png|gif|webp|jpg))$/i,
    isMod: false,
    release: '/static/widgets/$1',
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1'
}, {
    reg: /^\/widgets\/(.*)\.(?:html)$/i,
    release: '/static/widgets/$1' + '.html',
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1' + '.html',
}, {
    reg: /^\/widgets\/(.*)\.(?:json)$/i,
    release: '/static/widgets/$1' + '.json',
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1' + '.json',
}, {
    reg: /^\/widgets\/(.*)\.(eot|svg|ttf|woff)$/i,
    release: '/static/widgets/$1' + '.$2',
    url: '/fwmap/upload/dq_promotion/map/static/widgets/$1' + '.$2',
    useDomain: false
}, {
    //其他css文件
    reg: "**.css",
